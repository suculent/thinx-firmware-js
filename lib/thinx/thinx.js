...
var Thinx = (function() {
	...
	var _private = {
		...
		update_and_reboot: function(url) {
			...
			_private.shell("git pull origin master; node index.js");
			process.exit();
		},
		...
		parse: function(body) {
			...
			if ((typeof(update) !== "undefined") && update !== null) {
				...
				const udid = root["udid"];
				if ((typeof(udid) !== "undefined") && udid !== null) {
					_thinx_udid = udid;
					console.log("Update for thinx_udid: "+ _thinx_udid);
				}
				...
				if ((commit.equals(_thinx_commit_id)) && (version.equals(_thinx_version))) {
					if ((typeof(_available_update_url) != "undefined") && (_available_update_url != null)) {
						console.log("*TH: firmware has same thx_commit_id as current and update availability is stored. Firmware has been installed.");
						_available_update_url = null;
						notify_on_successful_update();
						return;
					} else {
						console.log(F("*TH: Info: firmware has same thx_commit_id as current and no update is available."));
					}
				}
				...
				if (_thinx_auto_update == false) {
					if (_mqtt_client != NULL) {
						...
						_public.publish(
							...
							{ title: "Update Available", body: "There is an update available for this device. Do you want to install it now?", type:"success" }
						);
					}
				...
				if ((type == "bool") || (type == "boolean")) {
					...
					if (response == true) {
						...
						if (_available_update_url) {
							update_and_reboot(_available_update_url);
						}
					} else {
