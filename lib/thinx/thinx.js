...
var Thinx = (function() {
	...
	if ( (typeof(process.env.DEVICE_ALIAS) !== "undefined") && (process.env.DEVICE_ALIAS != null) ) {
		defaults.thinx.device_alias = process.env.DEVICE_ALIAS;
	}
		...
		start_mqtt: function(start_callback) {
			...
			_mqtt_client.on('connect', function() {
				...
				_mqtt_client.on('message', function(topic, message) {
					...
					parse(message);
					if ((typeof(_mqtt_callback) !== "undefined") && _mqtt_callback !== null) {
			...
			var mdnsTimer = setTimeout(function() {
				...
				_private.start_mqtt(function() {
					...
					function checkinEvent() {
						...
						runloopTimer = setInterval(function() {
							checkinEvent();
						}, _checkin_interval);
					}
