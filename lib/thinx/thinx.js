...
var Thinx = (function() {
	...
	var _private = {
		...
		start_mqtt: function(start_callback) {
			...
			_mqtt_client.on('connect', function(data) {
				console.log("MQTT Connected: " + _mqtt_connected)
				if (!_mqtt_connected) {
		...
		finalize: function() {
			_phase = phase.COMPLETED;
			console.log("*TH: MQTT Init completed.");
	...
	var _public = {
		...
		initMqtt: function(mdnsTimer) {
			...
			_private.start_mqtt(function() {
				...
				function checkinEvent() {
					console.log("Loop-timer for check-in called: " + _checkinTimerCounter);
					_private.checkin();
				}

				if (typeof(_checkin_interval) !== "undefined" && typeof(_checkinTimer) == "undefined" && _checkin_interval > 0) {
					if (_checkin_interval >= 3) {
						console.log("Setting checkin interval: " + _checkin_interval + " sec");
						var _checkinTimerCounter = 0;
						_checkinTimer = setInterval(function() {
							_checkinTimerCounter++;
							checkinEvent();
						}, _checkin_interval * 1000);
					} else {
				...
				if (_reboot_interval != null && typeof(_rebootTimer) == "undefined") {
					if (_reboot_interval >= 30) {
						console.log("Setting reboot interval: " + _reboot_interval + " sec");
						_rebootTimer = setInterval(function() {
							rebootEvent();
						}, _reboot_interval * 1000);
					} else {
		...
		setFinalizeCallback: function(func) {
			console.log("setFinalizeCallback");
...
exports.setFinalizeCallback = Thinx.setFinalizeCallback;
