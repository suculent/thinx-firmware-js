...
var Thinx = (function() {
	...
	var _private = {
		...
		checkin: function() {
			_private.send_data(_private.checkin_body(), function() {
				console.log("Should send data: "+JSON.stringify(_private.checkin_body()));
			});
		},
		...
		send_data: function() {
			...
			console.log("HTTPS request with options: "+JSON.stringify(options));

			var rbody = _private.checkin_body();
			console.log("HTTPS request with body: "+JSON.stringify(rbody));
		...
		start_mqtt: function(finalize_callback) {
			...
			console.log("MQTT connect with options: " + JSON.stringify(options));
